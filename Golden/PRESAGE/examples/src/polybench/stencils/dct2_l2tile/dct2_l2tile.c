...
if (M >= 1) {
  for (t2=0;t2<=floord(M-1,256);t2++) {
    for (t3=0;t3<=floord(M-1,256);t3++) {
      for (t4=8*t2;t4<=min(floord(M-1,32),8*t2+7);t4++) {
        for (t5=8*t3;t5<=min(floord(M-1,32),8*t3+7);t5++) {
          for (t6=32*t4;t6<=min(M-1,32*t4+31);t6++) {
            lbv=32*t5;
...
#pragma vector always
  ...
  for (t2=0;t2<=floord(M-1,256);t2++) {
    for (t3=0;t3<=floord(M-1,256);t3++) {
      for (t5=0;t5<=floord(M-1,256);t5++) {
        for (t6=8*t2;t6<=min(floord(M-1,32),8*t2+7);t6++) {
          for (t7=8*t3;t7<=min(floord(M-1,32),8*t3+7);t7++) {
            for (t9=8*t5;t9<=min(floord(M-1,32),8*t5+7);t9++) {
              for (t10=32*t6;t10<=min(M-1,32*t6+31);t10++) {
                for (t11=32*t7;t11<=min(M-1,32*t7+31);t11++) {
                  for (t13=32*t9;t13<=min(M-1,32*t9+31);t13++) {
                    temp2d[(t11)*M + t10] = temp2d[(t11)*M + t10] + block[(t11)*M + t13] * cos1[(t10)*M + t13];;
                  }
                }
              }
            }
          ...
          for (t7=8*t3;t7<=min(floord(M-1,32),8*t3+7);t7++) {
            for (t9=8*t5;t9<=min(floord(M-1,32),8*t5+7);t9++) {
              for (t10=32*t6;t10<=min(M-1,32*t6+31);t10++) {
                for (t11=32*t7;t11<=min(M-1,32*t7+31);t11++) {
                  lbv=32*t9;
                  ubv=min(M-1,32*t9+31);
#pragma ivdep
#pragma vector always
                  for (t13=lbv;t13<=ubv;t13++) {
                    sum2[(t13)*M + t10] = sum2[(t13)*M + t10] + cos1[(t13)*M + t11] * temp2d[(t11)*M + t10];;
                  }
                }
              }
            }
