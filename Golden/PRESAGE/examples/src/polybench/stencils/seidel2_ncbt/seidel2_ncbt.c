...
if ((N >= 3) && (T >= 1)) {
  for (t1=0;t1<=T-1;t1++) {
    for (t2=t1+1;t2<=t1+N-2;t2++) {
      for (t3=t1+t2+1;t3<=t1+t2+N-2;t3++) {
        a[((-t1+t2))*N + (-t1-t2+t3)] = (a[((-t1+t2)-1)*N + (-t1-t2+t3)-1] + a[((-t1+t2)-1)*N + (-t1-t2+t3)] + a[((-t1+t2)-1)*N + (-t1-t2+t3)+1] + a[((-t1+t2))*N + (-t1-t2+t3)-1] + a[((-t1+t2))*N + (-t1-t2+t3)] + a[((-t1+t2))*N + (-t1-t2+t3)+1] + a[((-t1+t2)+1)*N + (-t1-t2+t3)-1] + a[((-t1+t2)+1)*N + (-t1-t2+t3)] + a[((-t1+t2)+1)*N + (-t1-t2+t3)+1])/9.0;;
      }
    }
  }
}
