...
/* We do not support C11 <threads.h>.  */
  int t1, t2, t2t, newlb_t2, newub_t2, t3, t3t, newlb_t3, newub_t3;
 register int lbv, ubv;
/* Start of CLooG code */
if ((N >= 4) && (T >= 1)) {
  if (N >= 5) {
    for (t1=0;t1<=T-1;t1++) {
      for (t3=2*t1+2;t3<=2*t1+N-2;t3++) {
        b[(2)*N + (-2*t1+t3)]= 0.2*(a[(2)*N + (-2*t1+t3)]+a[(2)*N + (-2*t1+t3)-1]+a[(2)*N + 1+(-2*t1+t3)]+a[(1+2)*N + (-2*t1+t3)]+a[(2 -1)*N + (-2*t1+t3)]);;
      }
      for (t2=2*t1+3;t2<=2*t1+N-2;t2++) {
        b[((-2*t1+t2))*N + 2]= 0.2*(a[((-2*t1+t2))*N + 2]+a[((-2*t1+t2))*N + 2 -1]+a[((-2*t1+t2))*N + 1+2]+a[(1+(-2*t1+t2))*N + 2]+a[((-2*t1+t2)-1)*N + 2]);;
        for (t3=2*t1+3;t3<=2*t1+N-2;t3++) {
          b[((-2*t1+t2))*N + (-2*t1+t3)]= 0.2*(a[((-2*t1+t2))*N + (-2*t1+t3)]+a[((-2*t1+t2))*N + (-2*t1+t3)-1]+a[((-2*t1+t2))*N + 1+(-2*t1+t3)]+a[(1+(-2*t1+t2))*N + (-2*t1+t3)]+a[((-2*t1+t2)-1)*N + (-2*t1+t3)]);;
          a[((-2*t1+t2-1))*N + (-2*t1+t3-1)]=b[((-2*t1+t2-1))*N + (-2*t1+t3-1)];;
        }
        a[((-2*t1+t2-1))*N + (N-2)]=b[((-2*t1+t2-1))*N + (N-2)];;
      }
      for (t3=2*t1+3;t3<=2*t1+N-1;t3++) {
        a[((N-2))*N + (-2*t1+t3-1)]=b[((N-2))*N + (-2*t1+t3-1)];;
      }
    }
  }
  if (N == 4) {
    for (t1=0;t1<=T-1;t1++) {
      b[(2)*N + 2]= 0.2*(a[(2)*N + 2]+a[(2)*N + 2 -1]+a[(2)*N + 1+2]+a[(1+2)*N + 2]+a[(2 -1)*N + 2]);;
      a[(2)*N + 2]=b[(2)*N + 2];;
    }
  }
}
